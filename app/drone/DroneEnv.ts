...
export class DroneEnv{
    ...
    constructor(droneSize: number, max_steps_per_episode: number, settings: DroneSettings){
    ...
    last_drone_position: {x: number, y: number, z: number} = {x: 0, y: 0, z: 0};
    ...
    step(action: number[], delta_time: number, position_scale: number){
        ...
        const dronePositionVector = new Vector3(this.drone.x, this.drone.y, this.drone.z);
        const goalPositionVector = new Vector3(this.goal.x, this.goal.y, this.goal.z);
        const droneToGoalVector = goalPositionVector.clone().sub(dronePositionVector).normalize();
        ...
        if(this.isFirstStep){
            ...
            this.last_drone_position = {x: this.drone.x, y: this.drone.y, z: this.drone.z};
        ...
        const getting_closer_to_goal = (last_distance_to_goal - distance_to_goal)/step_size;

        // const getting_further_from_goal = (distance_to_goal - last_distance_to_goal);


        const sensor_increase_right_raw = this.drone_sensor_right - this.last_drone_sensor_right;
        const sensor_increase_left_raw = this.drone_sensor_left - this.last_drone_sensor_left;
        const sensor_increase_front_raw = this.drone_sensor_front - this.last_drone_sensor_front;
        const sensor_increase_back_raw = this.drone_sensor_back - this.last_drone_sensor_back;
        const sensor_increase_below_raw = this.drone_sensor_below - this.last_drone_sensor_below;
        const sensor_increase_above_raw = this.drone_sensor_above - this.last_drone_sensor_above;


        const max_possible_sensor_increase = step_size/this.settings.maxSensorDistance;


        const sensor_increase_right = sensor_increase_right_raw/max_possible_sensor_increase;
        const sensor_increase_left = sensor_increase_left_raw/max_possible_sensor_increase;
        const sensor_increase_front = sensor_increase_front_raw/max_possible_sensor_increase;
        const sensor_increase_back = sensor_increase_back_raw/max_possible_sensor_increase;
        const sensor_increase_below = sensor_increase_below_raw/max_possible_sensor_increase;
        const sensor_increase_above = sensor_increase_above_raw/max_possible_sensor_increase;
        

        const sensor_increase = sensor_increase_right + sensor_increase_left + sensor_increase_front + sensor_increase_back + sensor_increase_below + sensor_increase_above;
        const sensor_increase_clipped = Math.max(0, sensor_increase_right) + Math.max(0, sensor_increase_left) + Math.max(0, sensor_increase_front) + Math.max(0, sensor_increase_back) + Math.max(0, sensor_increase_below) + Math.max(0, sensor_increase_above);




        const max_sensor_value = Math.max(this.drone_sensor_left, this.drone_sensor_right, this.drone_sensor_front, this.drone_sensor_back, this.drone_sensor_below, this.drone_sensor_above);
        ...
        if(hitTopWall&&explode){
            console.log('hit top wall');
        }

        if(hitBottomWall&&explode){
            console.log('hit bottom wall');
        }

        if(hitLeftWall&&explode){
            console.log('hit left wall');
        }

        if(hitRightWall&&explode){
            console.log('hit right wall');
        }

        if(hitFrontWall&&explode){
            console.log('hit front wall');
        }
        ...
        const explode_because_of_edge_walls = explode && (hitTopWall || hitBottomWall || hitLeftWall || hitRightWall || hitFrontWall || hitBackWall);

        if(explode && !explode_because_of_edge_walls){
            console.log('explode because of sensor');
        }

        const reached_goal = Math.sqrt(
            (this.drone.x - this.goal.x)**2 +
            (this.drone.y - this.goal.y)**2 +
            (this.drone.z - this.goal.z)**2
        ...
        //     return {
        //         state: newState,
        //         reward: explode_because_of_edge_walls ? -2 : -1,
        //         done: true
        //     }
        // }
        ...
        const step_vector_length = Math.sqrt(step_x**2 + step_y**2 + step_z**2);
        
        let dot = 0;
        if(step_vector_length > 0){
            const step_vector_normalized = {x: step_x/step_vector_length, y: step_y/step_vector_length, z: step_z/step_vector_length};
            const lastDronePositionToGoal = {x:  this.goal.x - this.last_drone_position.x, y: this.goal.y - this.last_drone_position.y, z: this.goal.z - this.last_drone_position.z};
            const lastDronePositionToGoalLength = Math.sqrt(lastDronePositionToGoal.x**2 + lastDronePositionToGoal.y**2 + lastDronePositionToGoal.z**2);
            dot = step_vector_normalized.x*lastDronePositionToGoal.x/lastDronePositionToGoalLength + step_vector_normalized.y*lastDronePositionToGoal.y/lastDronePositionToGoalLength + step_vector_normalized.z*lastDronePositionToGoal.z/lastDronePositionToGoalLength;
        }


        // const max_distance = Math.sqrt(3*(this.container_size/2)**2);
        const max_distance = this.start_distance_to_goal;
        //const closeness_to_goal = -distance_to_goal/max_distance;

        // const max_distance = 3*(this.container_size/2);
        // const closeness_to_goal = 1 - distance_to_goal/max_distance;

        // const getting_closer_to_goal = -getting_further_from_goal;

        const sensor_total = this.drone_sensor_left + this.drone_sensor_right + this.drone_sensor_front + this.drone_sensor_back + this.drone_sensor_below + this.drone_sensor_above;

        // const sensor_increase_penalty = -10*(sensor_increase_right + sensor_increase_left + sensor_increase_front + sensor_increase_back + sensor_increase_below + sensor_increase_above);
        //const reward = sensor_increase_penalty + closeness_to_goal;

        //const reward =  -0.1*distance_to_goal;

         //const reward = 10*(1.0*max_distance - distance_to_goal)/max_distance;

        // const reward = -0.1*distance_to_goal/max_distance + 3*dot - sensor_total;
        ...
        // const reward = this.settings.distancePenalty * distance_to_goal/max_distance + this.settings.directionReward * dot + this.settings.proximitySensorPenalty * sensor_total;

        // const reward = this.settings.distancePenalty * distance_to_goal/max_distance + 
        ...
        //const reward =  this.settings.proximitySensorPenalty * sensor_increase;
        ...
        // const reward = -0.1 * distance_to_goal/max_distance + 100*Math.max(0, getting_closer_to_goal);


        //const reward = -1 * distance_to_goal/max_distance;


        // const reward = 0*sensor_total -0.8*(1-closeness_to_goal) - 0*this.step_counter/this.max_steps_per_episode + 0*getting_closer_to_goal;

        //const reward = -0.5*max_sensor_value + closeness_to_goal + sensor_increase_penalty;
        //const reward = 1*Math.max(0,getting_closer_to_goal) - 400*sensor_increase - max_sensor_value + 0.5*closeness_to_goal;



        // const reward = -100*getting_further_from_goal - 400*sensor_increase + 0.5*closeness_to_goal;
        //const reward = -100*(getting_further_from_goal + 4*sensor_increase) ;
        //console.log('reward', reward.toFixed(4), 'getting_further_from_goal', getting_further_from_goal.toFixed(4), 'sensor_increase', sensor_increase.toFixed(4));
        ...
        return {
            ...
            color: color
            //done: this.episode_start_time > this.max_steps_per_episode
        }
    }
    ...
    convertSensorDistanceToSensorValue(distance: number){
        ...
        const sensorValue = 1 - (Math.min(distance, maxSensorDistance) / maxSensorDistance);
    ...
    reset(){
        ...
        //drone start is set to be at a point randomly spread out from [0,2,0]
        // const drone_x = this.centeredRandom(this.container_size*0.5);
        // const drone_y = this.centeredRandom(this.container_size*0.5);
        // const drone_z = (0.1 - 0.5)*this.container_size;
        ...
        // const goal_x = this.centeredRandom(this.container_size*0.5);
        // const goal_y = this.centeredRandom(this.container_size*0.5);
        // const goal_z = this.centeredRandom(this.container_size*0.5);
        //const goal_z = this.container_size*(0.7-0.5);
        ...
        while(go){
            iterations++;
            if(iterations > 2000){
                console.warn('failed to find a spot to place an obstacle after 2000 iterations');
                debugger;
                break;
            }
            ...
            const obstacle_radius = 2*Math.max(potential_obstacle.width/2, potential_obstacle.height/2, potential_obstacle.depth/2);
        ...
        // for(let i = 0; i < 4; i++){
        //     this.obstacles.push({
        //         x: this.centeredRandom(container_size/2),
        //         y: this.centeredRandom(container_size/2),
        //         z: this.centeredRandom(container_size/2),
        //         width: this.centeredRandom(2),
        //         height: this.centeredRandom(2),
        //         depth: this.centeredRandom(2)
        //     })
        // }

        const droneToGoalVector = new Vector3(this.goal.x - this.drone.x, this.goal.y - this.drone.y, this.goal.z - this.drone.z).normalize();
    ...
    boxOverlap(box1: {x: number, y: number, z: number, width: number, height: number, depth: number}, box2: {x: number, y: number, z: number, width: number, height: number, depth: number}){
    ...
    get_sensor_reading_box(position: Vector3, direction: directions, sensor_width: number, sensor_height: number, obstacle: DroneObstacle){
        const overhang = 1.3;
        ...
        //find closest corner
        const corners = [
            new Vector3(obstacle.x + obstacle.width/2, obstacle.y + obstacle.height/2, obstacle.z + obstacle.depth/2),
            new Vector3(obstacle.x - obstacle.width/2, obstacle.y + obstacle.height/2, obstacle.z + obstacle.depth/2),
            new Vector3(obstacle.x + obstacle.width/2, obstacle.y - obstacle.height/2, obstacle.z + obstacle.depth/2),
            new Vector3(obstacle.x - obstacle.width/2, obstacle.y - obstacle.height/2, obstacle.z + obstacle.depth/2),
            new Vector3(obstacle.x + obstacle.width/2, obstacle.y + obstacle.height/2, obstacle.z - obstacle.depth/2),
        ...
        const point_to_plane_dot = point_to_corner.clone().dot(direction_vector.clone().normalize());




        //project point_to_corner onto direction
        // const point_to_plane = point_to_corner.clone().projectOnVector(direction_vector.clone().normalize());
    ...
    get_sensor_reading4(pos_x: number, pos_y: number, pos_z: number, direction_x: number, direction_y: number, direction_z: number, obstacle: {x: number, y: number, z: number, width: number, height: number, depth: number}){
        const point = new Vector3(pos_x, pos_y, pos_z);

        const front_face = {
            tl: new Vector3(obstacle.x + obstacle.width/2, obstacle.y + obstacle.height/2, obstacle.z + obstacle.depth/2),
            tr: new Vector3(obstacle.x - obstacle.width/2, obstacle.y + obstacle.height/2, obstacle.z + obstacle.depth/2),
            bl: new Vector3(obstacle.x + obstacle.width/2, obstacle.y - obstacle.height/2, obstacle.z + obstacle.depth/2),
            br: new Vector3(obstacle.x - obstacle.width/2, obstacle.y - obstacle.height/2, obstacle.z + obstacle.depth/2)
        };
        ...
        const closest_point_front = this.get_closest_point_between_point_and_face(point, front_face);
        const closest_point_back = this.get_closest_point_between_point_and_face(point, back_face);
        const closest_point_right = this.get_closest_point_between_point_and_face(point, right_face);
        const closest_point_left = this.get_closest_point_between_point_and_face(point, left_face);
        const closest_point_top = this.get_closest_point_between_point_and_face(point, top_face);
        const closest_point_bottom = this.get_closest_point_between_point_and_face(point, bottom_face);
        ...
        const angle_front = this.get_angle_between_vectors(direction_vector, pointToClosestFront);
        const angle_back = this.get_angle_between_vectors(direction_vector, pointToClosestBack);
        const angle_right = this.get_angle_between_vectors(direction_vector, pointToClosestRight);
        const angle_left = this.get_angle_between_vectors(direction_vector, pointToClosestLeft);
        const angle_top = this.get_angle_between_vectors(direction_vector, pointToClosestTop);
        const angle_bottom = this.get_angle_between_vectors(direction_vector, pointToClosestBottom);
        ...
        // const min_closest_point = Math.min(closest_point_front.distanceTo(point), closest_point_back.distanceTo(point), closest_point_right.distanceTo(point), closest_point_left.distanceTo(point), closest_point_top.distanceTo(point), closest_point_bottom.distanceTo(point));

        // return Math.max(0, min_closest_point);
    
    }

    get_angle_between_vectors(vector1: Vector3, vector2: Vector3){
        const dot_product = vector1.dot(vector2);
        const angle = Math.acos(dot_product / (vector1.length() * vector2.length()));
        return angle;
    }

    
    get_sensor_reading2(pos_x: number, pos_y: number, pos_z: number, direction_x: number, direction_y: number, direction_z: number, obstacle: {x: number, y: number, z: number, width: number, height: number, depth: number}){
        ...
        const length = Math.sqrt(direction_x * direction_x + direction_y * direction_y + direction_z * direction_z);
        direction_x /= length;
        direction_y /= length;
        direction_z /= length;
        ...
        if (intersect_x < obstacle_left || intersect_x > obstacle_right ||
            intersect_y < obstacle_bottom || intersect_y > obstacle_top ||
            intersect_z < obstacle_front || intersect_z > obstacle_back) {
            return Infinity;
        }
    ...
    get_closest_point_between_point_and_face(point: Vector3, face: {tl: Vector3, tr: Vector3, bl: Vector3, br: Vector3}){
        ...
        const face_right = new Vector3(face.br.x - face.bl.x, face.br.y - face.bl.y, face.br.z - face.bl.z);
        const face_up = new Vector3(face.tl.x - face.bl.x, face.tl.y - face.bl.y, face.tl.z - face.bl.z);
        ...
        const projected_point = point.clone().sub(face_normal.clone().multiplyScalar(distance_to_plane));
        ...
        const local_x = projected_point.clone().sub(face_center).dot(face_right_normalized);
        const local_y = projected_point.clone().sub(face_center).dot(face_up_normalized);
        ...
        for (const edge of edges) {
            ...
            const clamped_projection = Math.max(0, Math.min(edge_length, projection_length));
            
            // Calculate closest point on this edge
            const closest_on_edge = edge.start.clone().add(edge_normalized.clone().multiplyScalar(clamped_projection));
    ...
    update_drone_sensor_readings(){
        ...
        for(let i = 0; i < this.obstacles.length; i++){
            ...
            const distance_left = this.get_sensor_reading_box(
                new Vector3(this.drone.x- this.droneSize/2, this.drone.y, this.drone.z),
                "left",
                this.droneSize,
                0.1*this.droneSize,
                obstacle
            ...
            const distance_front = this.get_sensor_reading_box(
                new Vector3(this.drone.x, this.drone.y, this.drone.z+ this.droneSize/2),
                "front",
                this.droneSize,
                0.1*this.droneSize,
                obstacle
            ...
            const distance_below = this.get_sensor_reading_box(
                new Vector3(this.drone.x, this.drone.y- 0.1*this.droneSize/2, this.drone.z),
                "bottom",
                this.droneSize,
                this.droneSize,
                obstacle
            ...
            // const distance_left = this.get_sensor_reading(
            ...
            // const distance_right = this.get_sensor_reading(
            //     this.drone.x+ this.droneSize/2,
            //     this.drone.y,
            //     this.drone.z,
            //     1,
            ...
            // const distance_front = this.get_sensor_reading(
            //     this.drone.x,
            //     this.drone.y,
            ...
            // const distance_back = this.get_sensor_reading(
            //     this.drone.x,
            //     this.drone.y,
            //     this.drone.z- this.droneSize/2,
            //     0,
            ...
            // const distance_below = this.get_sensor_reading(
            //     this.drone.x,
            ...
            // const distance_above = this.get_sensor_reading(
            //     this.drone.x,
            //     this.drone.y+ 0.1*this.droneSize/2,
            //     this.drone.z,
            //     0,
        ...
        this.drone_sensor_left = this.convertSensorDistanceToSensorValue(min_distance_left);
        this.drone_sensor_right = this.convertSensorDistanceToSensorValue(min_distance_right);
        this.drone_sensor_front = this.convertSensorDistanceToSensorValue(min_distance_front);
        this.drone_sensor_back = this.convertSensorDistanceToSensorValue(min_distance_back);
        this.drone_sensor_below = this.convertSensorDistanceToSensorValue(min_distance_below);
        this.drone_sensor_above = this.convertSensorDistanceToSensorValue(min_distance_above);
